...
async function deleteRecipeById(id) {
    const res = await query("DELETE FROM recipes WHERE id = $1 RETURNING *;", [id]);
    return res;
}
...
async function postNewRecipe(recipeObject) {
    ...
    const res = await query("INSERT INTO recipes (country, recipe, ingredients, steps) VALUES ($1,$2,$3,$4) RETURNING *;", [country, recipe, ingredients, steps]);
    return res;
}
...
async function updateRecipeById(recipeObject, id) {
    const { country, recipe, ingredients, steps } = recipeObject;
    const res = await query("UPDATE recipes SET country = $1, recipe = $2, ingredients = $3, steps = $4) WHERE id = $5 RETURNING *;", [country, recipe, ingredients, steps, id]);
    return res;
}
...
async function patchRecipeById(col_name, value, id) {
    const data = await query(`UPDATE recipe SET ${col_name} = $1 WHERE id = $2 RETURNING *;`, [value, id]);
    return data.rows;
}
